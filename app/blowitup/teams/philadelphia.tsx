    ...
    <section className='w-full'>
      <div className='flex flex-col'>
        ...
        <div className='w-20 h-20 relative mx-auto md:h-0 md:w-0 mb-6 md:mb-0'>
          ...
          <p className="md:hover:scale-105 md:mr-3 m-0 w-0 h-0 md:h-auto md:p-0 md:w-auto flex justify-center items-center rounded-full md:rounded-lg text-sm border-neutral-200 text-neutral-100 no-underline bg-gradient-to-r from-blue-900 via-white to-red-800">
            <NBAIcons.PHI/>
          </p>
          <div className="w-full mr-3 items-center p-1 text-sm leading-4 bg-gray-300 text-black no-underline overflow-hidden border border-neutral-200 rounded-lg opacity-80 transition duration-300 ease-in-out group-hover:opacity-100">
            <h3 className='px-2 md:px-4 text-lg font-semibold p-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-900 via-red-800 to-white'>Philadelphia 76ers</h3>
            <p className='px-2 md:px-4 pb-2 leading-5 text-[0.8rem] font-semibold'>{``}</p>
            <p className='px-2 md:px-4 pb-2 leading-5 text-[0.8rem] font-semibold'>{``}</p>
            <Accordion title="LeBron James for... ">
              <div className='flex flex-col md:flex-row'>
                <div className='flex flex-col'>
                  <p className='mb-3 mt-3 px-4 md:pr-2 md:pb-2 text-[0.8rem] font-semibold'><strong className='bg-clip-text text-transparent bg-gradient-to-r from-blue-900 via-red-800 to-white'>{`Bradley Beal.`}</strong>  {`In this scenario Atlanta drafted Bronny to try and lure LeBron into a Draymond role but it wasn't enough.`}</p>
